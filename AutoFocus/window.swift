...
struct AccessibilityHelper {
    ...
    static func raiseAndFocusWindow(at point: CGPoint) throws {
        ...
        guard result == .success, let leafElement = elementRaw else {
...
extension AXUIElement {
    ...
    func findParent(role: CFString) -> AXUIElement? {
        // 自分自身が対象ならそれを返す
        if self.getRole() == role { return self }
        ...
        while let parent = currentElement.getParent() {
            ...
            if currentElement.getRole()! as String == kAXSystemWideRole {
                break
            }
        }
        return nil
    ...
    func getRole() -> CFString? {
        ...
        return result == .success ? (roleRaw as! CFString) : nil
    }
    ...
    func getParent() -> AXUIElement? {
        ...
        if result == .success, let parent = parentRaw {
            // CFTypeRefをAXUIElementにキャスト
